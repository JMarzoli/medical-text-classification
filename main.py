...
lr_prediction = [classification.logistic_regression(X_train, X_test, y_train), 'logistic-regression']
nb_prediction = [classification.naive_bayes(X_train, X_test, y_train), 'naive-bayes']
svm_prediction = [classification.support_vector_machine(X_train, X_test, y_train), 'support-vector-machine']
rf_prediction = [classification.random_forest(X_train, X_test, y_train), 'random-forest']
dt_prediction = [classification.decision_tree(X_train, X_test, y_train), 'decision-tree']
...
graphic.plot_confusion_matrices(predictions, y_test, category_list)
# saving the model reports
graphic.log_model_reports(predictions, y_test, category_list, txt=True, csv=True)
